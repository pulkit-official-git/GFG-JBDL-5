...
public class TransactionService {
    ...
    @Value("${library.student.daysLimit}")
...
//            rollback mechanism
    ...
    public int calculateFine(Book book, Student student) {
        ...
        Long issuedTxnTimeInMillis = issuedTxn.getUpdatedOn().getTime();

        Long timeDifferenceInMillis = System.currentTimeMillis() - issuedTxnTimeInMillis;

        Long timeDifferenceInDays = TimeUnit.DAYS.convert(timeDifferenceInMillis, TimeUnit.MILLISECONDS);

        if(timeDifferenceInDays > studentDaysLimit){
            return (timeDifferenceInDays.intValue() - studentDaysLimit)*finePerDay;
        }
        return 0;
